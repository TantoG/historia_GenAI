...
export const SlideContent: React.FC<SlideContentProps> = ({ data, onInteract }) => {
  return (
    <div className="flex flex-col items-center w-full h-full text-center">
      ...
      {data.researcher && (
          <div className="flex items-center gap-4 bg-white p-3 rounded-full shadow-md mb-6 animate-fade-in-down">
              <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center text-5xl border-2 border-blue-500 overflow-hidden shrink-0">
                {data.researcher.imageUrl ? <img src={data.researcher.imageUrl} alt={data.researcher.name} className="w-full h-full object-cover"/> : "🧑‍🔬"}
              </div>
              <div className="text-left pr-4">
                  <h3 className="font-bold text-slate-800 text-sm leading-tight">{data.researcher.name}</h3>
                  <p className="text-blue-600 text-xs font-medium">{data.researcher.role}</p>
              </div>
          </div>
      )}
      ...
      {data.image && (
        <img 
          src={data.image} 
          alt={data.title} 
          className="max-w-xl mx-auto rounded-lg shadow-md mb-8"
        />
      )}
